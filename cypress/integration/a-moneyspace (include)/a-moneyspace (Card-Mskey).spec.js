...
let payment = "card-mskey";
let order_id = ms.tests.CardMskey.orderIdPrefix + Date.now();
...
var ms_data = {
    ...
    "body": {
        ...
        "firstname": "My410fc447s4",
        "lastname": "mYd7qEbstwySdri1cN",
        "email": "test@test.com",
        "phone": "0888888888",
        "amount": "1.8339",
        "description": "gPK1DbT250zLZd4qHRQf!1AB1Stf7gN",
        "address": "111/11",
        "message": "J7Mh4teVwE",
        "feeType": "include",
        "order_id": order_id,
...
describe('Card-Mskey ( Include )', () => {
    ...
    context('Check Fail URL', () => {
        ...
        //         url: "https://www.moneyspace.net/merchantapi/fail?Ref=" + ms_data.transactionID_Stage,
        //     }).then(response => {
        ...
        //         cy.Check_Tx("prod", "check-tx-whitelable", ms_data.transactionID_Stage).then(response => {
        //             expect(response.body[0]["transaction id"]).to.have.all.keys('status', 'amount', 'description')
        ...
        //             cy.log(JSON.stringify(response.body))
        //         });
        //         cy.Check_Tx("prod", "check-tx-phpsdk", ms_data.transactionID_Stage).then(response => {
        //             expect(response.body[0]).to.have.all.keys("Amount ", "Transaction ID ", "Description ", "Status Payment ")
